...
(function () {
          ...
          <div class="mt-4">
            <div>
              <div class="mt-1 relative rounded-md shadow-sm">
                <input type="text" name="cronjob_setup" id="cronjob_setup" class="font-mono focus:ring-as-primary-400 focus:border-as-primary-400 block w-full pr-10 text-xs border-gray-300 rounded-md" value="curl -s -m 1 ${module_cron_url} > /dev/null 2>&1">
  ...
  $(document).ready(function () {
    ...
    $("#send_as_queries").on("click", function (e) {
      e.preventDefault();
      const pid = $("#as_pid_query").val();
      if (!pid) return alert("Inserisci un ID prodotto");
      AS.controller("sendasqueries", "POST", {
        pid,
      }).then((r) => {
        console.log(r);
      });
